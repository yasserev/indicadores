document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set default to current week (let's say week 10 for demo purposes)
  weekSelector.value = "10";

  // Chart instances to destroy and recreate them
  let productionChartInstance = null;
  let qualityChartInstance = null;

  // 2. Data Generators (Mocking real-time or historical data)
  function generateMockData(week) {
    // Seed based on the week so selecting the same week yields the same data
    ...
    return {
      ...
      quality: {
        ...
        trend: -(Math.sin(week) * 1.5).toFixed(1), // Less is better
      },
      oee: {
      ...
      dailyProduction: [
        ...
        Math.round((baseProduction / 6) * 0.5), // Sunday
      ],
      qualityDistribution: [
        Math.round(baseYield), // Exportable
        Math.round(baseQuality), // Descarte
        100 - Math.round(baseYield) - Math.round(baseQuality), // Nacional/Mercado interno
      ],
      lines: [
        {
          id: "Línea 1 - Palta",
          status: week % 3 === 0 ? "warning" : "active",
          eff: (baseOee + 2).toFixed(1),
          batch: "L-2026-A1X",
        },
        {
          id: "Línea 2 - Arándano",
          status: "active",
          eff: (baseOee + 5).toFixed(1),
          batch: "L-2026-B3Y",
        },
        {
          id: "Línea 3 - Mango",
          status: week % 5 === 0 ? "stopped" : "active",
          eff: week % 5 === 0 ? "0.0" : (baseOee - 4).toFixed(1),
          batch: week % 5 === 0 ? "Ninguno" : "L-2026-C2Z",
        },
        {
          id: "Línea 4 - Empaque",
          status: "active",
          eff: (baseOee + 1).toFixed(1),
          batch: "Múltiple",
        },
      ],
    };
  ...
  function updateKPIs(data) {
    // Production
    animateValue(
      document.getElementById("kpi-production"),
      0,
      data.production.current,
      1000,
    );
    updateTrend(
      "trend-container-production",
      "trend-production",
    ...
    // Yield
    animateValue(
      document.getElementById("kpi-yield"),
      0,
      parseFloat(data.yield.current),
      1000,
    );
    updateTrend("trend-container-yield", "trend-yield", data.yield.trend, true);

    // Quality (Mermas - lower is better)
    animateValue(
      document.getElementById("kpi-quality"),
      0,
      parseFloat(data.quality.current),
      1000,
    );
    updateTrend(
      "trend-container-quality",
      "trend-quality",
      data.quality.trend,
      false,
    );
    ...
    // OEE
    animateValue(
      document.getElementById("kpi-oee"),
      0,
      parseFloat(data.oee.current),
      1000,
    );
  ...
  function updateCharts(data) {
    // Destroy existing to prevent overlay issues
    ...
    gradientBlue.addColorStop(0, "rgba(0, 210, 255, 0.8)");
    gradientBlue.addColorStop(1, "rgba(0, 210, 255, 0.1)");
  ...
  function updateTable(lines) {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.status === "active") {
        ...
        statusText =
          '<i class="fa-solid fa-triangle-exclamation"></i> Mant. Preventivo';
      } else {
      ...
      tr.innerHTML = `
                ...
                <td><span style="font-family: monospace; color: var(--accent-blue);">${line.batch}</span></td>
            `;
  ...
  weekSelector.addEventListener("change", () => {
    // Add a small delay to simulate network fetch and allow CSS transitions to trigger
